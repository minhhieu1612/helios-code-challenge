...
Implement a robust, production-grade ExpressJS + TypeScript backend service managing `Student` resources (containing `name`, `dateOfBirth`, `gender`) backed by MongoDB, with full Swagger API documentation, Postman collection, an Agent Skill for API docs synchronization, and comprehensive setup & inventory documentation.

## User Review Required

> [!NOTE]
> MongoDB database connection options: The service will connect to a local MongoDB instance (`mongodb://localhost:27017/student_db`) by default or an external MongoDB Atlas cluster via the `MONGODB_URI` environment variable in `.env`.
...
- Define scripts: `dev`, `build`, `start`, `sync:docs`.
...
### Agent Skill & Synchronization Tools
...
- Comprehensive guide covering:
  - Configuration & Environment Setup (Express server, MongoDB connection)
  - Guidelines to run the application (`npm run dev`)
  ...
  - **Inventory Ownership Report**:
    ...
    - **AI-Generated Files**: All source code (`src/*`), `package.json`, `tsconfig.json`, `postman_collection.json`, `.agents/skills/sync-api-docs/SKILL.md`, `.gitignore`, `README.md`.
...
![alt text](feedback-from-first-attempt.png)
also need to provide mongodb server setup (through CLI)
